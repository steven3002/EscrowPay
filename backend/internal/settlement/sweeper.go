...
type Driver interface {
	...
	SweepInflightSettlementLegs(ctx context.Context) (int, error)
...
func (s *Sweeper) Tick(ctx context.Context) Report {
	...
	r.LegsResolved = s.run(ctx, "resolve_inflight_legs", s.driver.SweepInflightSettlementLegs)
	...
	if n := r.Total(); n > 0 {
		s.logger.Info("sweeper tick",
			...
			slog.Int("legs_resolved", r.LegsResolved),
